...
2. A script for the comparison of the organization of a genome or genomes against a reference based on the annotations (Promer_style_genome_comparison.pyde).
This Processing v3.5.4 (Python mode) script was developed for comparing the genome organization of Sulcia and Vidania symbionts, after we concluded that PROMER - a standard tool for such comparisons - is struggling to identify extremely divergent regions corresponding to the same gene.
 
Before running the script, you need to provide:
- A list of genes divided into functional categories
- A list of genes and their positions in the query genome, in a 'PRO' format obtained by parsing an annotation GFF file using 'process_gff.py' script; 
- GFF files of annotated genomes
- A list of genes and their positions in the query genome, in a 'PRO' format
 
To convert GFF files into .pro files, use the enclosed process_gff.py script.
./process_gff.py [gff_file] > [pro_file], e.g.,
./process_gff.py VFDICMUL.gff > VFDICMUL.pro
 
After that, ensure that within the body of the Promer_style_genome_comparison.pyde script, you refer to the correct files.
...
This Processing v3.5.4 (Python mode) script was developed to visualize symbiont contributions to amino acid biosynthesis pathways. It generates a grid-style figure with color-coded pie charts representing the symbionts involved in amino acid biosynthesis. Each row in the grid represents a species, each column represents a gene, and the size of each pie-chart is proportional to the numer of symbionts whose genome encodes that gene.
...
- Merged data from all sources (list of all possible metagenome/gene combinations with information on the symbiont genome where the gene is present, if any)
Examples of all required files are available in the folder Nutritional_complementarity_files.


4. A Processing script for the visualization of the symbiont genome contents table (Genome_content_visualisation.pyde)
